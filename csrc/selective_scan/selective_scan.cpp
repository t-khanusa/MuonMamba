 ...
 ******************************************************************************/
 ...
 selective_scan_fwd(const at::Tensor &u, const at::Tensor &delta,
     ...
     // Debug: fprintf(stderr, "[CPP] beta=%.6f, use_newton_schulz=%d\n", beta, use_newton_schulz);
     // fflush(stderr);
     
     if (use_newton_schulz) {
         // X_4_buffer stores intermediate state before 5th NS iteration
         // Shape: [batch, dim, seqlen, dstate] in float32
         // For complex weights, we store only the real part for now
     ...
     // Return X_4_buffer so it can be saved for backward pass
     if (use_newton_schulz) { result.push_back(X_4_buffer); }
     return result;
 }
 ...
 selective_scan_bwd(const at::Tensor &u, const at::Tensor &delta,
                   ...
                   float beta,
                   float alpha,
                   const c10::optional<at::Tensor> &X_4_buffer_) {
     auto input_type = u.scalar_type();
     auto weight_type = A.scalar_type();
     TORCH_CHECK(input_type == at::ScalarType::Float || input_type == at::ScalarType::Half || input_type == at::ScalarType::BFloat16);
     TORCH_CHECK(weight_type == at::ScalarType::Float || weight_type == at::ScalarType::ComplexFloat);
     ...
     // For NS mode: main backward doesn't compute full gradients for du/ddelta/dB
     // They're computed by NS backward and added later, so initialize to zero
     const bool use_newton_schulz = (beta != 0.0f);
     at::Tensor du = torch::zeros_like(u);  // Always zeros - NS backward will add to it
     at::Tensor ddelta = torch::zeros_like(delta);  // Always zeros - NS backward will add to it
     ...
     // Allocate gradient buffer for Newton-Schulz backward pass if needed
     at::Tensor grad_X_4_buffer;
     at::Tensor X_4_buffer;  // Forward pass buffer (b_t_ortho values from NS forward)
     at::Tensor du_ns_temp, ddelta_ns_temp, dB_ns_temp;  // Temporary float32 buffers for NS backward
     at::Tensor X_temp, dX_4_temp;  // Pre-allocated temporary buffers for NS backward recomputation
     if (use_newton_schulz) {
         // grad_X_4_buffer stores gradients w.r.t. the orthogonalized B (output of NS)
         // Shape: [batch, dim, seqlen, dstate] in float32
         // For complex weights, we store only the real part for now
         grad_X_4_buffer = torch::zeros({batch_size, dim, seqlen, dstate}, 
                                        u.options().dtype(at::ScalarType::Float));
         // X_4_buffer from forward pass - contains b_t_ortho (X_5 from NS)
         // CRITICAL: Use the saved X_4_buffer from forward pass, NOT a new empty tensor!
         if (X_4_buffer_.has_value()) {
             X_4_buffer = X_4_buffer_.value();
         } else {
             // Fallback: allocate empty (will cause incorrect gradients!)
             // This should not happen if forward pass properly saved X_4_buffer
             fprintf(stderr, "WARNING: X_4_buffer not provided from forward pass! Gradients will be incorrect.\n");
             X_4_buffer = torch::empty({batch_size, dim, seqlen, dstate}, 
                                      u.options().dtype(at::ScalarType::Float));
         }
         
         // Allocate temporary float32 buffers for NS backward gradients
         // NS backward writes float32, we'll convert and add to final gradients later
         du_ns_temp = torch::zeros_like(du, du.options().dtype(at::ScalarType::Float));
         ddelta_ns_temp = torch::zeros_like(ddelta, ddelta.options().dtype(at::ScalarType::Float));
         dB_ns_temp = torch::zeros_like(dB, dB.options().dtype(at::ScalarType::Float));
         
         // Pre-allocate temporary buffers for NS backward recomputation
         // CRITICAL: These are allocated ONCE per backward pass, not inside the launch wrapper
         // This avoids host-synchronizing cudaMalloc/cudaFree calls during training
         X_temp = torch::empty({batch_size, dim, seqlen, dstate}, 
                              u.options().dtype(at::ScalarType::Float));
         dX_4_temp = torch::zeros({batch_size, dim, seqlen, dstate}, 
                                  u.options().dtype(at::ScalarType::Float));
     }
     ...
     // Set Newton-Schulz parameters for backward pass
     params.use_newton_schulz = use_newton_schulz;
     params.ns_steps = 5;
     params.X_4_buffer_ptr = use_newton_schulz ? X_4_buffer.data_ptr() : nullptr;
     params.grad_X_4_buffer_ptr = use_newton_schulz ? grad_X_4_buffer.data_ptr() : nullptr;
     params.du_ns_temp_ptr = use_newton_schulz ? du_ns_temp.data_ptr() : nullptr;
     params.ddelta_ns_temp_ptr = use_newton_schulz ? ddelta_ns_temp.data_ptr() : nullptr;
     params.dB_ns_temp_ptr = use_newton_schulz ? dB_ns_temp.data_ptr() : nullptr;
     params.X_temp_ptr = use_newton_schulz ? X_temp.data_ptr() : nullptr;
     params.dX_4_temp_ptr = use_newton_schulz ? dX_4_temp.data_ptr() : nullptr;
     ...
     // If Newton-Schulz was used, accumulate temporary float32 gradients into final gradient tensors
     if (use_newton_schulz) {
         // Convert and add NS gradients: final_grad += NS_temp_grad
         du.add_(du_ns_temp.to(du.dtype()));
         ddelta.add_(ddelta_ns_temp.to(ddelta.dtype()));
         dB.add_(dB_ns_temp.to(dB.dtype()));
     }
