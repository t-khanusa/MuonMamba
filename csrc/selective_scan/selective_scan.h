 ...
 ******************************************************************************/
 ...
 struct SSMParamsBase {
     ...
     // X_4_buffer stores the intermediate state before the 5th NS iteration for backward pass
 ...
 struct SSMParamsBwd: public SSMParamsBase {
     ...
     // Newton-Schulz gradient buffers: [batch, dim, seqlen, dstate]
     // Gradient buffer for X_4 intermediate state
     void *__restrict__ grad_X_4_buffer_ptr;
     
     // Temporary float32 buffers for NS backward gradients
     // NS backward writes float32, we convert and add to final gradients
     void *__restrict__ du_ns_temp_ptr;
     void *__restrict__ ddelta_ns_temp_ptr;
     void *__restrict__ dB_ns_temp_ptr;
     
     // Pre-allocated temporary buffers for NS backward recomputation
     // These avoid host-synchronizing cudaMalloc/cudaFree calls during training
     void *__restrict__ X_temp_ptr;      // [batch, dim, seqlen, dstate] - for X_4 recomputation
     void *__restrict__ dX_4_temp_ptr;   // [batch, dim, seqlen, dstate] - for dX_4 gradients
 };
