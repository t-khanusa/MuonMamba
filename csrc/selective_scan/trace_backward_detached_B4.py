#!/usr/bin/env python3
"""
Backward pass with B_4 DETACHED (matching CUDA's approach)
"""
...
print("=" * 80)
print("Backward Pass with DETACHED B_4 (matching CUDA)")
print("=" * 80)
...
print(f"B_4[0,:4] = {B_4[0,:4]} (detached)")
...
print(f"\ndX_4 (with detached B_4):")
print(f"dX_4[0,:4] = {dX_4[0,:4]}")

print(f"\nCUDA dX_4:")
print(f"dX_4[0,:4] = [-0.152178, 0.023382, 0.220887, 0.019735]")

print(f"\nDifferences:")
for i in range(4):
    cuda = [-0.152178, 0.023382, 0.220887, 0.019735][i]
    py = dX_4[0,i].item()
    diff = abs(py-cuda)
    match = "✓" if diff < 1e-4 else "✗"
    print(f"  dX_4[0,{i}]: Python={py:.6f}, CUDA={cuda:.6f}, diff={diff:.6f} {match}")
