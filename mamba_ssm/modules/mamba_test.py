...
class MomentumMambaConfig:
    ...
    # Momentum parameters
    momentum_beta: float = 0.6  # β - momentum decay factor (recommended: 0.5-0.9)
    momentum_alpha: float = 1.0  # α - momentum input scaling (recommended: 0.5-1.5)
...
class MomentumMamba(nn.Module):
    """Multi-layer Momentum Mamba model with residual connections"""
    def __init__(self, config: MomentumMambaConfig):
        super().__init__()
    ...
    def step(self, x, caches):
        """
        Inference step for autoregressive generation
        x : (B, L, D)
        caches : [cache(layer) for all layers]
        
        Returns: (B, L, D), updated caches
        """
...
class ResidualBlock(nn.Module):
    """Residual block with Mamba mixer and RMSNorm"""
    def __init__(self, config: MomentumMambaConfig, layer_idx: int = None):
        super().__init__()
        
        self.mixer = MomentumMambaBlock(config, layer_idx=layer_idx)
...
class MomentumMambaBlock(nn.Module):
    """Mamba block with momentum - wraps the original Mamba implementation"""
    def __init__(self, config: MomentumMambaConfig, layer_idx: int = None):
        super().__init__()
...
class Mamba(nn.Module):
    def __init__(
        ...
        device=None,
        dtype=None,
        beta=0.99,  # Momentum decay parameter
        alpha=0.6,  # Momentum scale parameter
    ):
        ...
        # Warning for numerical stability
        if beta > 0.95:
            import warnings
            warnings.warn(
                f"High momentum beta={beta} may cause numerical instability (NaN/Inf) "
                f"especially with mixed precision training. Recommended beta < 0.9 for stability. "
                f"Consider using beta=0.0 (no momentum) if you encounter NaN/Inf issues.",
                UserWarning
            )
